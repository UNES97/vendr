...
<div class="max-w-full px-4 lg:px-8">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-gray-600 mt-1">Analyze your sales performance and trends</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white rounded-lg shadow p-6 border-l-4 border-red-600 hover:shadow-lg transition">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-2"><?php echo get_currency(); ?> <span id="total-sales"><?php echo number_format($total_sales, 0); ?></span></p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow p-6 border-l-4 border-blue-600 hover:shadow-lg transition">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-2"><span id="total-orders"><?php echo $total_orders; ?></span></p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow p-6 border-l-4 border-green-600 hover:shadow-lg transition">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-gray-600 text-sm font-medium">Average Order</p>
                    <p class="text-2xl font-bold text-gray-900 mt-2"><?php echo get_currency(); ?> <span id="avg-order"><?php echo number_format($average_order, 0); ?></span></p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow p-6 border-l-4 border-yellow-600 hover:shadow-lg transition">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-2"><?php echo get_currency(); ?> <span id="today-sales">0</span></p>
                </div>
    ...
    <div class="bg-white rounded-lg p-6 mb-6 border border-gray-200">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            <div>
                ...
                <select id="report-payment-method" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-600 focus:border-transparent outline-none transition">
            ...
            <div class="flex items-end gap-2">
                <button onclick="loadSalesReport()" class="bg-red-600 hover:bg-red-700 text-white font-semibold px-4 py-2 rounded-lg transition text-sm flex-1">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-white rounded-lg shadow p-6 border border-gray-200">
            <h3 class="text-lg font-bold text-gray-900 mb-4">Sales by Payment Method</h3>
        ...
        <!-- Daily Sales Chart -->
        <div class="bg-white rounded-lg shadow p-6 border border-gray-200">
            <h3 class="text-lg font-bold text-gray-900 mb-4">Daily Sales Trend</h3>
            <div style="height: 300px;">
...
function loadSalesReport() {
    ...
    const url = new URL('<?php echo base_url('reports/get_sales_data'); ?>', window.location.origin);
...
function updateSalesCards(data) {
    document.getElementById('total-sales').textContent =
        parseFloat(data.total_sales || 0).toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    document.getElementById('total-orders').textContent = data.total_orders || 0;
    document.getElementById('avg-order').textContent =
        parseFloat(data.average_order || 0).toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
}
...
function updateDailyChart(dailySales) {
    ...
    const amounts = dailySales.map(d => parseFloat(d.total) || 0);
...
function displayPage() {
    ...
    if (!allOrders || allOrders.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" class="px-6 py-8 text-center text-gray-500"><p class="text-sm">No orders found</p></td></tr>';
    ...
    pageOrders.forEach(order => {
        ...
        row.innerHTML = `
            ...
            <td class="px-6 py-4 text-sm text-gray-600">${new Date(order.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</td>
            <td class="px-6 py-4 text-sm text-gray-600">${order.item_count || 0}</td>
            <td class="px-6 py-4 text-sm font-semibold text-right text-gray-900">${CURRENCY} ${parseFloat(order.total_amount).toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</td>
            <td class="px-6 py-4 text-sm"><span class="px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800 uppercase">${order.payment_method || 'N/A'}</span></td>
            <td class="px-6 py-4 text-sm"><span class="px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">${order.order_status || 'Pending'}</span></td>
        `;
...
function updatePaginationControls() {
    ...
    const endIdx = Math.min(currentPage * ITEMS_PER_PAGE, allOrders.length);

    document.getElementById('pagination-start').textContent = allOrders.length === 0 ? 0 : startIdx;
    document.getElementById('pagination-end').textContent = endIdx;
    document.getElementById('pagination-total').textContent = allOrders.length;
